...
public final class PNAndTreeLearner extends LearnerBase {
	...
	private void _learnNode(AOTree node) {
		...
		if(!nodeReady) {
			...
			outer:
			for(Var s : scopedVars) { for(Var p : scopedVars) { for(Var o : scopedVars) {
		...
		if(!nodeReady) {
			...
			//			System.out.println("Learner: start (v,v,c)");
			outer:
				for(Var s : scopedVars) { for(Var p : scopedVars) {
		...
		if(!nodeReady) {
			...
			outer:
				for(Var s : scopedVars) { for(Var o : scopedVars) {
		...
		if(!nodeReady) {
			...
			outer:
				for(Var p : scopedVars) { for(Var o : scopedVars) {
