...
public abstract class UtilsJena {
	 ...
	 *
	 * @param triple {@code Triple} object to process.
...
//	}
	 ...
	 * @param sol mapping from variables to nodes to apply, if possible.
	 */
	private static Node looseEval(Node var, Map<Var, Node> sol) {
...
//	}
	 ...
	 *
	 * @param tree {@code AOTree} to be converted.
	 * @return an {@code Op} representing the same query.
	 */
	public static Op convertAOTreeToOp(AOTree tree) {
		...
		for(AOTree child : tree.getChildren()) {
			op = OpLeftJoin.create(op, convertAOTreeToOp(child), ExprList.emptyList);
		}
	 ...
	 * @param solution a {@code QuerySolution} object.
	 * @return an ASK (i.e. boolean) {@code Query} which will test whether {@code solution} is in the answer to
     * {@code query}.
	 */
